...
namespace FireCubeBase
{
    ...
    public class SpatialAnchorLoader : MonoBehaviour
    {
        ...
        /// 单个锚点加载完成的事件
        /// </summary>
        public Action<Anchor> LoadAction;
        ...
        public void LoadAnchorsByUuid(string uuid)
        {
            var uuids = AnchorUuidStore.Uuids.ToArray();
            if (uuids.Length == 0)
            {
            ...
            List<Guid> localGuids = new List<Guid>();//定位锚点只允许一个

            foreach (Guid guid in uuids)
            {
                if (guid.ToString() == uuid)
                {
                    localGuids.Add(guid);
                    Debug.Log($"找到了定位锚点");
                    break;
            ...
            OVRSpatialAnchor.LoadUnboundAnchorsAsync(localGuids, _unboundAnchors)
                .ContinueWith(result =>
                {
                    if (result.Success)
                    {
        ...
        private void OnLocalized(bool success, OVRSpatialAnchor.UnboundAnchor unboundAnchor)
        {
            ...
            if (spatialAnchor.TryGetComponent<Anchor>(out var anchor))
            {
                ...
                if (LoadAction != null) LoadAction(anchor);
            }
        }
