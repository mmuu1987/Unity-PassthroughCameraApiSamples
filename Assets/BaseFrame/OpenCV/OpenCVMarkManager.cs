...
{
    public class OpenCVMarkManager : MonoBehaviour
    {

        public static OpenCVMarkManager Instance;
        /// <summary>
        ...
        public OpenCVAnchor OpenCvAnchor { get; private set; }


        private void Awake()
        {
            if (Instance != null) throw new UnityException($"{this.GetType()} 单例已经存在");
        ...
        private IEnumerator Start()
        {
            ...
            if (m_webCamTextureManager == null)
            {
                Debug.LogError($"PCA: {nameof(m_webCamTextureManager)} field is required " +
                            $"for the component {nameof(OpenCVMarkManager)} to operate properly");
                enabled = false;
                yield break;
            }
        ...
        private void CvAnchor_TrackSuccessEvent(Transform obj,bool isAndroid)
        {
            isTrackSuccess = true;
        }
        ...
        private void HandleVisualizationToggle()
        {
            ...
            {
                float length = m_arucoMarkerTracking.GetLength() + 0.01f;


                m_arucoMarkerTracking.SetLength(length);

                Debug.Log($"增加长度，增加后的长度：{length}");
            }
        ...
        public void Reset()
        {
            m_cameraAnchor.position = Vector3.zero;
            m_cameraAnchor.rotation = Quaternion.identity;
