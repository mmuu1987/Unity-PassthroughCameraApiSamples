...
namespace TryAR.MarkerTracking
{
    ...
    public class ChArUcoTrackingAppCoordinator : MonoBehaviour
    {
        ...
        private void HandleVisualizationToggle()
        {
            ...
            {
                float length = m_charucoMarkerTracking.GetLength() + 0.01f;
                m_charucoMarkerTracking.SetLength(length);

                float square = m_charucoMarkerTracking.GetSquareLength() + 0.01f;
                m_charucoMarkerTracking.SetSquareLength(square);

                Debug.Log($"增加长度，增加后的长度：markLength:{length}  squareLength:{square}");

                m_charucoMarkerTracking.UpdateBoard();
            }
            ...
            if (OVRInput.GetDown(OVRInput.RawButton.B))
            {
                ...
                float square = m_charucoMarkerTracking.GetSquareLength() - 0.01f;
                if (square <= 0.03f) square = 0.03f;
                m_charucoMarkerTracking.SetSquareLength(square);



                Debug.Log($"减少长度，减少后的长度：markLength:{length}  squareLength:{square}");
